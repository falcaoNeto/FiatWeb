...
class ClienteController:
    ...
    def ListarClientes(self):
        id_func = 3
        try:
            # 'dados' deve ser uma lista de dicionários com as chaves definidas na query
            dados = Cliente.GetClientes(id_func)
            clientes_dict = {}
            filhos_list = []
            ...
            for row in dados:
                ...
                if id_cliente not in clientes_dict:
                    clientes_dict[id_cliente] = {
                        ...
                        "cpf": row["cpf"],
                        "nome_cliente": row["nome_cliente"],         # Usando alias
                ...
                # Verifica se há informação de filho (id_filho não é None)
                if row["id_filho"] is not None:
                    filho = {
                        "id_filho": row["id_filho"],
                        "nome_filho": row["nome_filho"],  # Usando alias para o nome do filho
                        "data_nascimento": format_data(row["data_nascimento_filho"]),
                        "id_cliente": row["id_cliente"]    # Relaciona o filho ao cliente
                    }
...
if __name__ == "__main__":
    ...
    e,r,t,y,u = controller.ListarClientes()
    print(e,r,t,y,u)